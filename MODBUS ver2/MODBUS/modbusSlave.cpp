...
void modbusSlave::REDEpinSet(uint8_t re,uint8_t de){
	REpin = re;
	DEpin = de;
	//ledPin = led;
...
/*
Retrieve the Serial2 baud rate
*/
...
*/
void modbusSlave::checkSerial2(void)
{
...
*/
void modbusSlave::Serial2Rx(void)
{
	byte i;
...
void modbusSlave::run(void)
{
	...
	//retrieve the query message from the Serial2 uart
	this->Serial2Rx();
...
//digitalWrite(ledPin,HIGH);
	...
	if(_len)
	{
		...
		int i;
		//send the reply to the Serial2 UART
		//Senguino doesn't support a bulk Serial2 write command....
		for(i = 0 ; i < _len ; i++)
